...
gene identifiers. Therefore, Bioconductor annotation package hgu133plus2.db will be 
used to convert probe identifiers from GPL570 platform to their corresponding 
Ensembl identifiers before analysis. Moreover, microarray platforms may contain
more than one probe for the same gene. This ambiguity will be removed by selecting 
probes with the highest mean of expression.

```{r}
library(hgu133plus2.db)
eslist <- as.list(hgu133plus2ENSEMBL)  
# Some probes are assigned to more than one Ensembl Gene ID.
# To simplify matters, these probes will not be included.
eslist <- lapply(eslist, function(es_id){ 
                   if(length(es_id) > 1){ 
                     return(NA) 
                   }else{
                     return(es_id)
                   }})
annottab <- data.frame(Probe = names(eslist), Ensembl = unname(unlist(eslist)))
annottab <- annottab[complete.cases(annottab),]
annotatb <- annottab[annottab$Probe %in% rownames(exp_matrix),]
genes2probes <- split(annottab$Probe, annottab$Ensembl)
...
diffexp <- data.frame(Ensembl = rownames(diffexp),
                 log2FC = diffexp$logFC,
                 pvalue = diffexp$P.Value,
                 CI.L = diffexp$CI.L,
                 CI.R = diffexp$CI.R)
```
Dataframe in "diffexp" variable can now be stored in a list. Results from other differential 
expression analyses from similarly designed experiments must be stored 
in the same list for meta-analysis.

```{r}
diffexplist <- list(GSE30999 = diffexp)
```
...
Finnaly, a second table with results can be added to a list for meta-analysis

```{r}
diffexplist[['E-GEOD-54456']] <- diffexp2
```
